...
# IMPORT ONLY THE ENGINE - DO NOT IMPORT SessionLocal or ProScanResult
...
def send_email(setups):
    ...
    html += "</table><p>Visit Dashboard for full ATR analysis and Stop Loss levels.</p>"
...
def run_automation():
    try:
        ...
        # Delete only today's data so re-runs are always safe and idempotent.
        # Hardcoded dates removed — they become dead code after that date passes.
        print(f"Maintenance: Purging existing data for {today}...")
            ...
            try:
                ...
                if len(df) >= 150:
                    ...
                    if score >= 70:
                        ...
                        if levels:
                            # CREATE RAW DICTIONARY (No ORM Class)
        ...
        # 4. DIRECT BATCH INSERT (Session-less & Unbreakable)
        if final_data_batch:
            print(f"Directly saving {len(final_data_batch)} setups to database...")
            # ON CONFLICT uses the named constraint for rock-solid duplicate protection.
            # The DELETE above already cleared today's rows, so this is a belt-and-suspenders guard.
            insert_query = text("""
            ...
            print("No setups met the score threshold today.")
        
        # 5. FETCH RESULTS FOR EMAIL (Pure SQL Read)
